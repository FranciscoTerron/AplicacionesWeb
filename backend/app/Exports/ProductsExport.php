...
class ProductsExport implements FromCollection, ShouldAutoSize, WithHeadings, WithStyles
{
    ...
    public function collection()
    {
        ...
        return collect($this->fetchAllDocumentsFast('products'))->map(function ($doc) {
            return [
                $doc['id'] ?? '',
                $doc['name'] ?? '',
                $doc['slug'] ?? '',
    ...
    {
        return ['ID', 'Nombre', 'Slug', 'SKU', 'Descripción', 'Precio', 'Stock', 'Estado', 'Categoría ID', 'Subcategoría ID', 'Creado', 'Actualizado'];
    }
    ...
    public function styles(Worksheet $sheet)
    {
        ...
        $sheet->getStyle('F2:F'.$sheet->getHighestRow())->getAlignment()->setHorizontal(Alignment::HORIZONTAL_RIGHT);
    }
